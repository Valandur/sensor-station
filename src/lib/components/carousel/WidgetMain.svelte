<script lang="ts">
	...
	import type { CarouselScreen } from '$lib/models/carousel';
	...
	import type { WidgetProps } from '$lib/models/widget';

	export let index: number;
	export let screen: CarouselScreen;
	export let nextScreen: string;
	export let prevScreen: string;
	export let nextPage: string;
	export let prevPage: string;
	export let switchInterval: number;
	export let updateInterval: number;
	export let props: WidgetProps;

	export let modem: ModemData;
	export let battery: BatteryData;
	export let holiday: HolidayServiceData;
	...
	$: if (browser) {
		...
		if (!$paused && nextScreen) {
			const next = nextScreen;
			start(() => goto(next), switchInterval);
		} else if ($paused) {
			timer = setInterval(() => invalidate('carousel'), updateInterval);
		}
	}
	...
	function onSwipe(e: SwipeEvent) {
		console.log(e.detail.dir, nextPage);
		if (e.detail.dir === 'left' && nextScreen) {
			goto(nextScreen);
		} else if (e.detail.dir === 'right' && prevScreen) {
			goto(prevScreen);
		} else if (e.detail.dir === 'up' && nextPage) {
			goto(nextPage);
		} else if (e.detail.dir === 'down' && prevPage) {
			goto(prevPage);
		}
	}
</script>
		...
		{#key index}
			<div class="h-100 w-100 m-0 p-1 position-absolute overflow-hidden" transition:fade>
				{#if screen.widget}
					...
					<svelte:component this={comps.main} {...props} />
				{:else}
