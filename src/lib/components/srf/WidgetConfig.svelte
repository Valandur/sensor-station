<script lang="ts">
	...
	import { SRF_SERVICE_TYPE, type SrfWidgetConfig } from '$lib/models/srf';

	export let name: string;
	export let config: SrfWidgetConfig;
	export let services: ServiceInstance[];

	$: validServices = services.filter((s) => s.type === SRF_SERVICE_TYPE);
</script>
