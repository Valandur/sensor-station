<script lang="ts">
	...
	import { EPIC_GAMES_SERVICE_TYPE, type EpicGamesWidgetConfig } from '$lib/models/epic-games';

	export let name: string;
	export let config: EpicGamesWidgetConfig;
	export let services: ServiceInstance[];

	$: validServices = services.filter((s) => s.type === EPIC_GAMES_SERVICE_TYPE);
</script>
