<script lang="ts">
	...
	import { PRUSA_SERVICE_TYPE, type PrusaWidgetConfig } from '$lib/models/prusa';

	export let name: string;
	export let config: PrusaWidgetConfig;
	export let services: ServiceInstance[];

	$: validServices = services.filter((s) => s.type === PRUSA_SERVICE_TYPE);
</script>
