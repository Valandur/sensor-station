<script lang="ts">
	...
	import { POST_SERVICE_TYPE, type PostWidgetConfig } from '$lib/models/post';

	export let name: string;
	export let config: PostWidgetConfig;
	export let services: ServiceInstance[];

	$: validServices = services.filter((s) => s.type === POST_SERVICE_TYPE);
</script>
