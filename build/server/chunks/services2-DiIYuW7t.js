import { B as BATTERY_SERVICE_TYPE, C as CALENDAR_SERVICE_TYPE, a as CAROUSEL_SERVICE_TYPE, E as EPIC_GAMES_SERVICE_TYPE, G as GALLERY_SERVICE_TYPE, H as HOLIDAY_SERVICE_TYPE, M as MODEM_SERVICE_TYPE, N as NETWORK_SERVICE_TYPE, P as PRUSA_SERVICE_TYPE, S as SBB_ALERTS_SERVICE_TYPE, b as SBB_DEPARTURES_SERVICE_TYPE, c as SRF_SERVICE_TYPE, d as SWISS_POST_SERVICE_TYPE, T as TUYA_SERVICE_TYPE, W as WEATHER_SERVICE_TYPE } from './services-BJig70jM.js';
import { f as assign, c as create_ssr_component, d as escape, v as validate_component, i as identity, n as noop, g as compute_slots, b as add_attribute, a as subscribe, e as each, o as onDestroy, m as missing_component, h as add_styles } from './ssr-BXLnvVnr.js';
import { P as PageTitle, E as ErrorCard, n as navigating } from './PageTitle-BMwv6NOy.js';
import { formatInTimeZone } from 'date-fns-tz';
import { de } from 'date-fns/locale';
import { isSameDay } from 'date-fns/isSameDay';
import { w as writable, r as readable, D as DEV } from './index-BhqCiM8-.js';
...
const PageLayout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.show === void 0 && $$bindings.show && show !== void 0)
    $$bindings.show(show);
  return `${show ? `<div class="container-fluid vh-100 d-flex flex-column py-1">${validate_component(PageTitle, "PageTitle").$$render($$result, { showReload: true, closeUrl }, {}, {
    center: () => {
...
const Preview$1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let fault;
  let powerIn;
  let powerIn5vIo;
  let { info } = $$props;
...
const Icon$2 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${battery ? `<div class="col-auto">${battery.charge > 70 ? `<i class="icofont-battery-full"></i>` : `${battery.charge > 40 ? `<i class="icofont-battery-half"></i>` : `${battery.charge > 10 ? `<i class="icofont-battery-low"></i>` : `<i class="icofont-battery-empty"></i>`}`}`} ${escape(battery.charge)}%</div>` : ``} ${battery?.state.includes("CHARGING") ? `<div class="col-auto" data-svelte-h="svelte-eltrex"><i class="icofont-plugin"></i></div>` : ``}`;
});
...
const Service$d = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${validate_component(Events, "Events").$$render($$result, { events: data.events }, {}, {})}` : `${data.type === "config" ? `${form?.message ? `${validate_component(ErrorCard, "ErrorCard").$$render($$result, { message: form.message }, {}, {})}` : `${form?.success ? `<div class="alert alert-success m-0" data-svelte-h="svelte-1cil5pf">Config saved!</div>` : ``}`} ${validate_component(ServiceConfig$c, "ServiceConfig").$$render($$result, { name, data }, {}, {})}` : `${validate_component(ErrorCard, "ErrorCard").$$render(
          $$result,
          {
            title: "Calendar",
            message: "Unknown action",
...
const ServiceConfig$b = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="row overflow-auto"><div class="col"><table class="table"><colgroup data-svelte-h="svelte-18ehtnz"><col width="40%"> <col width="25%"> <col width="25%"> <col></colgroup> <thead data-svelte-h="svelte-1qnn2ry"><tr><th>Service</th> <th>Action</th> <th>Icon</th> <th></th></tr></thead> <tbody><tr class="bg-dark"><td><select form="formNew" name="service" class="form-select"><option value="" selected disabled data-svelte-h="svelte-1d2mk1m">---</option>${each(services, (service) => {
    return `<option${add_attribute("value", service.name, 0)}>${escape(service.name)} [${escape(service.type.name)}]</option>`;
  })}</select></td> <td><select form="formNew" name="action" class="form-select"><option value="" data-svelte-h="svelte-dz2bat">---</option>${each(actions, (action) => {
    return `<option${add_attribute("value", action, 0)}>${escape(action)}</option>`;
  })}</select></td> <td><select form="formNew" name="icon" class="form-select"><option value="" data-svelte-h="svelte-dz2bat">---</option>${each(actions, (action) => {
    return `<option${add_attribute("value", action, 0)}>${escape(action)}</option>`;
  })}</select></td> <td><form id="formNew" method="POST"><input type="hidden" name="name"${add_attribute("value", name, 0)}> <input type="hidden" name="__formAction" value="add"> <button type="submit" class="btn btn-theme" ${"disabled"}><i class="icofont-ui-add"></i></button></form></td></tr> ${each(screens, (screen, index) => {
    return `<tr><td>${escape(screen.name)}</td> <td>${escape(screen.action ?? "---")}</td> <td>${escape(screen.icon ?? "---")}</td> <td><form method="POST"><input type="hidden" name="name"${add_attribute("value", name, 0)}> <input type="hidden" name="__formAction" value="delete"> <input type="hidden" name="index"${add_attribute("value", index, 0)}> <div class="btn-group" data-svelte-h="svelte-1acy9t1"><button class="btn btn-danger"><i class="icofont-ui-delete"></i> </button></div> </form></td> </tr>`;
  })}</tbody></table></div></div>`;
});
...
const Carousel = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="container-fluid vh-100 d-flex flex-column"><div class="row flex-nowrap mb-2 p-1"><div role="presentation" class="col-auto d-flex flex-row align-items-end mt-1 p-0"><div class="time-main svelte-k0npo4">${escape(timeStr)}</div> <div class="time-seconds align-self-stretch d-flex flex-column justify-content-between ms-2 svelte-k0npo4"><div class="svelte-k0npo4">${escape(secondStr)}</div> <div class="text-muted text-nowrap svelte-k0npo4">${escape(tzStr)}</div></div></div> <div class="col d-flex flex-column justify-content-end align-items-end p-0"><div class="row icons flex-nowrap justify-content-end svelte-k0npo4">${each(icons, (icon) => {
  ...
  })} ${$paused ? `<div class="col-auto" data-svelte-h="svelte-1ggu063"><i class="icofont-ui-pause"></i></div>` : ``}</div> <div class="row flex-nowrap"><div class="h2 col text-nowrap m-0">${escape(date)}</div></div> <div class="row align-items-center flex-nowrap"> <div class="col-auto fw-bold text-white text-nowrap m-0">${escape(dateSub)}</div></div></div></div> <div class="row flex-fill position-relative"><div class="h-100 w-100 m-0 p-1 position-absolute overflow-hidden">${screen ? (() => {
    ...
    return ` ${validate_component(comp || missing_component, "svelte:component").$$render(
      ...
      {}
						...
						to add some.</p>`}</div></div></div> <div class="progress bg-secondary svelte-k0npo4"${add_styles({ "width": $progress + "%" })}></div> ${$navigating ? `<div class="loading svelte-k0npo4" data-svelte-h="svelte-bod3bb"><i class="icofont-spinner"></i></div>` : ``}`;
});
...
const Games = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_tz();
  return `<div class="h-100 d-flex flex-column justify-content-end">${games.length > 0 ? `<div class="row row-cols-2 overflow-auto">${each(games, (game) => {
...
const Service$b = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${validate_component(Games, "Games").$$render($$result, { games: data.games }, {}, {})}` : `${data.type === "config" ? `${form?.message ? `${validate_component(ErrorCard, "ErrorCard").$$render($$result, { message: form.message }, {}, {})}` : `${form?.success ? `<div class="alert alert-success m-0" data-svelte-h="svelte-1cil5pf">Config saved!</div>` : ``}`} ${validate_component(ServiceConfig$a, "ServiceConfig").$$render($$result, { name, data }, {}, {})}` : `${validate_component(ErrorCard, "ErrorCard").$$render(
          $$result,
          {
            title: "Epic Games",
            message: "Unknown action",
...
const Service$a = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${validate_component(Gallery, "Gallery").$$render($$result, { image: data.image }, {}, {})}` : `${data.type === "config" ? `${form?.message ? `${validate_component(ErrorCard, "ErrorCard").$$render($$result, { message: form.message }, {}, {})}` : ``} ${validate_component(ServiceConfig$9, "ServiceConfig").$$render($$result, { name, data }, {}, {})}` : `${validate_component(ErrorCard, "ErrorCard").$$render(
          $$result,
          {
            title: "Gallery",
            message: "Unknown action",
...
const Service$9 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${validate_component(Holiday, "Holiday").$$render($$result, { holiday: data.holiday }, {}, {})}` : `${data.type === "config" ? `${form?.message ? `${validate_component(ErrorCard, "ErrorCard").$$render($$result, { message: form.message }, {}, {})}` : `${form?.success ? `<div class="alert alert-success m-0" data-svelte-h="svelte-1cil5pf">Config saved!</div>` : ``}`} ${validate_component(ServiceConfig$8, "ServiceConfig").$$render($$result, { name, data }, {}, {})}` : `${validate_component(ErrorCard, "ErrorCard").$$render(
          $$result,
          {
            title: "Swiss Post",
            message: "Unknown action",
...
const Icon$1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${modem?.cellular.operator ? `<div class="col-auto"><i class="icofont-globe"></i> ${escape(modem.cellular.operator.split(" ", 2)[0])}</div>` : ``} ${modem?.cellular.signal ? `<div class="col-auto"><i class="icofont-signal"></i> ${escape(modem.cellular.signal.toFixed(0))}%</div>` : ``} ${modem?.gps ? `<div class="col-auto"><i class="icofont-satellite"></i> ${escape(modem.gps.lat.toFixed(2))} | ${escape(modem.gps.lng.toFixed(2))}</div>` : `${modem?.geo ? `<div class="col-auto"><i class="icofont-world"></i> ${escape(modem.geo.lat.toFixed(2))} | ${escape(modem.geo.lng.toFixed(2))}</div>` : ``}`}`;
});
const Service$8 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${action === "icon" ? `${validate_component(Icon$1, "Icon").$$render($$result, { modem: data.info }, {}, {})}` : `${validate_component(Preview, "Preview").$$render($$result, { info: data.info }, {}, {})}`}` : `${data.type === "config" ? `${form?.message ? `${validate_component(ErrorCard, "ErrorCard").$$render($$result, { message: form.message }, {}, {})}` : `${form?.success ? `<div class="alert alert-success m-0" data-svelte-h="svelte-1cil5pf">Config saved!</div>` : ``}`} ${validate_component(ServiceConfig$7, "ServiceConfig").$$render($$result, { name, data }, {}, {})}` : `${validate_component(ErrorCard, "ErrorCard").$$render(
          $$result,
          {
            title: "Battery",
            message: "Unknown action",
...
const Icon = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${connected ? `<i class="icofont-link"></i>` : `<i class="icofont-link-broken"></i>`}`;
});
...
const Service$7 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${action === "icon" ? `${validate_component(Icon, "Icon").$$render($$result, { connected: data.connected }, {}, {})}` : `${validate_component(Interfaces, "Interfaces").$$render($$result, { interfaces: data.interfaces }, {}, {})}`}` : `${validate_component(ErrorCard, "ErrorCard").$$render(
          $$result,
          {
            title: "Gallery",
            message: "Unknown action",
...
const Printer = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="h-100 d-flex flex-column justify-content-end">${job ? `<div class="row"><div class="col">${validate_component(Card, "Card").$$render($$result, {}, {}, {
    header: () => {
...
const Service$5 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${validate_component(Alert, "Alert").$$render($$result, { alert: data.alert }, {}, {})}` : `${data.type === "config" ? `${form?.message ? `${validate_component(ErrorCard, "ErrorCard").$$render($$result, { message: form.message }, {}, {})}` : `${form?.success ? `<div class="alert alert-success m-0" data-svelte-h="svelte-1cil5pf">Config saved!</div>` : ``}`} ${validate_component(ServiceConfig$5, "ServiceConfig").$$render($$result, { name, data }, {}, {})}` : `${validate_component(ErrorCard, "ErrorCard").$$render(
          $$result,
          {
            title: "SBB Alerts",
            message: "Unknown action",
...
const Service$4 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${validate_component(Departures, "Departures").$$render($$result, { departures: data.departures }, {}, {})}` : `${data.type === "config" ? `${form?.message ? `${validate_component(ErrorCard, "ErrorCard").$$render($$result, { message: form.message }, {}, {})}` : `${form?.success ? `<div class="alert alert-success m-0" data-svelte-h="svelte-1cil5pf">Config saved!</div>` : ``}`} ${validate_component(ServiceConfig$4, "ServiceConfig").$$render($$result, { name, data }, {}, {})}` : `${validate_component(ErrorCard, "ErrorCard").$$render(
          $$result,
          {
            title: "SBB Departures",
            message: "Unknown action",
...
const News = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="h-100 d-flex flex-column">${``} ${articles.length > 0 ? `${each(articles, (item) => {
...
const Service$3 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${validate_component(News, "News").$$render($$result, { name, articles: data.articles }, {}, {})}` : `${data.type === "details" ? `${validate_component(Details, "Details").$$render(
          $$result,
          {
            head: data.head,
            body: data.body,
...
const Shipment = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="h-100 d-flex flex-column justify-content-end">${shipment ? `<div class="row"><div class="col">${validate_component(Card, "Card").$$render($$result, {}, {}, {
    ...
    header: () => {
      return `<div>${escape(shipment.number)}</div> ${shipment.arrival ? `<div><i class="icofont-calendar"></i> ${escape(formatInTimeZone(parseISO(shipment.arrival), $tz, "dd.MM.yy", { locale: de$1 }))}</div>` : ``} `;
    },
    default: () => {
...
const Service$2 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render(
    ...
    {
      default: () => {
        return `${data ? `${data.type === "data" ? `${validate_component(Shipment, "Shipment").$$render($$result, { shipment: data.shipment }, {}, {})}` : `${data.type === "config" ? `${form?.message ? `${validate_component(ErrorCard, "ErrorCard").$$render($$result, { message: form.message }, {}, {})}` : `${form?.success ? `<div class="alert alert-success m-0" data-svelte-h="svelte-1cil5pf">Config saved!</div>` : ``}`} ${validate_component(ServiceConfig$2, "ServiceConfig").$$render($$result, { name, data }, {}, {})}` : `${validate_component(ErrorCard, "ErrorCard").$$render(
          $$result,
          {
            title: "Swiss Post",
            message: "Unknown action",
...
const Marker = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="row d-flex flex-row justify-content-end"><div class="col-auto text-muted"><i class="icofont-location-pin"></i> ${location.place ? `${escape(location.place)}` : `${escape(location.lat)}, ${escape(location.lng)}`}</div></div>`;
});
const css$1 = {
...
const Daily = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_tz();
  return `<div class="h-100 d-flex flex-column justify-content-between">${validate_component(Marker, "Marker").$$render($$result, { location }, {}, {})} <div class="row flex-nowrap">${each(daily, (forecast) => {
...
const Hourly = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  $$unsubscribe_tz();
  return `<div class="h-100 d-flex flex-column justify-content-between">${validate_component(Marker, "Marker").$$render($$result, { location }, {}, {})} <div class="row flex-nowrap">${each(hourly, (forecast) => {
...
const Alerts = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.alert === void 0 && $$bindings.alert && alert !== void 0)
    $$bindings.alert(alert);
  $$unsubscribe_tz();
  return `<div class="h-100 d-flex flex-column justify-content-between">${validate_component(Marker, "Marker").$$render($$result, { location }, {}, {})} ${alert ? `<div class="row mh-100 overflow-hidden"><div class="col mh-100">${validate_component(Card, "Card").$$render($$result, { type: "warning" }, {}, {
    subTitle: () => {
    ...
    header: () => {
      return `<div>${escape(alert.tags)}</div> <div><i class="icofont-calendar"></i> ${escape(formatInTimeZone(alert.start, $tz, "dd.MM.yy HH:mm", { locale: de }))} -
    ...
    {},
    {
      default: () => {
        return `${data ? `${data.type === "hourly" ? `${validate_component(Hourly, "Hourly").$$render(
          ...
          {},
          {}
        )}` : `${data.type === "alerts" ? `${validate_component(Alerts, "Alerts").$$render(
          $$result,
          {
            location: data.location,
            alert: data.alert
...
const SERVICES = {
  ...
  [EPIC_GAMES_SERVICE_TYPE]: Service$b,
  [GALLERY_SERVICE_TYPE]: Service$a,
  [HOLIDAY_SERVICE_TYPE]: Service$9,
...
export { Card as C, PageLayout as P, SERVICES as S };
//# sourceMappingURL=services2-DiIYuW7t.js.map
