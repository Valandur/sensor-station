...
import { mkdir, readFile, writeFile, rm, stat as stat$1 } from 'fs/promises';
import { i as isHttpError, e as error, f as fail, r as redirect, b as private_env } from './shared-server-C8XaTXK4.js';
import { format } from 'date-fns/format';
import { inspect } from 'node:util';
import chalk from 'chalk';
import { parse, parseISO as parseISO$1, differenceInMinutes, differenceInSeconds } from 'date-fns';
import { mkdir as mkdir$1, stat, readdir, rm as rm$1, readFile as readFile$1, writeFile as writeFile$1 } from 'node:fs/promises';
...
const MODEM_SERVICE_ACTIONS = ["main", "debug", "icon", "config"];
    ...
    const data = await this.cache.with(
      ...
      async () => {
        ...
        for (const event of items) {
          ...
          events2.push({
            tsStart: parseISO$1(start),
            tsEnd: parseISO$1(end),
...
class CarouselService extends BaseService {
  ...
  async getData(action, { url, cookies }) {
    ...
    const service = servicesService.getByName(screen.name);
...
class DhtSensorService extends BaseService {
  ...
  async getData({ url }) {
    ...
    const data = await this.cache.with(
      ...
      async () => {
        if (!await stat$1(this.config.devicePath).catch(() => null)) {
      ...
      async () => {
        ...
        for (const game of rawGames) {
          ...
          const offers = pos.flatMap((po) => po.promotionalOffers).concat(game.promotions.upcomingPromotionalOffers.flatMap((po) => po.promotionalOffers)).filter((po) => po.discountSetting.discountType === "PERCENTAGE").map((po) => ({
            start: parseISO$1(po.startDate),
            end: po.endDate ? parseISO$1(po.endDate) : null,
  ...
  async setConfig({ form }) {
    ...
    switch (formAction) {
      case "add": {
        ...
        const newDate = parseISO$1(newDateStr);
...
class Device2 {
  ...
  async getCellularTimeAndTz() {
    ...
    if (cclkMatch) {
      ...
      const time = parseISO$1(`${year}-${month}-${day}T${hour}:${minute}:${second}${tz}`);
      return [time, tz];
    }
    return [null, null];
  }
    ...
    const data = await this.cache.with(
      ...
      async () => {
        ...
        for (const sit of sits) {
          ...
          alerts.push({
            start: parseISO$1(sit.ValidityPeriod[0].StartTime[0]),
            end: parseISO$1(sit.ValidityPeriod[0].EndTime[0]),
    ...
    const data = await this.cache.with(
      ...
      async () => {
        ...
        for (const d of stopEventResponse["trias:StopEventResult"]) {
          ...
          const scheduled = parseISO$1(scheduledStr);
          const estimatedStr = serviceDeparture?.["trias:EstimatedTime"]?.[0];
          const estimated = estimatedStr ? parseISO$1(estimatedStr) : null;
      ...
      async () => {
        ...
        for (const fileName of fileNames) {
          if (!articles2.some((item) => item.id === fileName)) {
            await rm$1(`${folderPath}/${fileName}`, { recursive: true, force: true });
          }
        }
        return {
          ts: /* @__PURE__ */ new Date(),
    ...
    const data = await this.cache.with(
      ...
      async () => {
        ...
        for (const rawShipment of rawShipments) {
          ...
          const events = eventsBody.sort((a, b) => parseISO(b.timestamp).getTime() - parseISO(a.timestamp).getTime()).map((e) => ({
...
export { BATTERY_SERVICE_TYPE as B, CAROUSEL_SERVICE_TYPE as C, DHT_SENSOR_SERVICE_TYPE as D, EPIC_GAMES_SERVICE_TYPE as E, GALLERY_SERVICE_TYPE as G, Logger as L, MODEM_SERVICE_TYPE as M, NETWORK_SERVICE_TYPE as N, PRUSA_SERVICE_TYPE as P, SWISS_POST_SERVICE_TYPE as S, TUYA_SERVICE_TYPE as T, WEATHER_SERVICE_TYPE as W, SRF_SERVICE_TYPE as a, SBB_DEPARTURES_SERVICE_TYPE as b, SBB_ALERTS_SERVICE_TYPE as c, CALENDAR_SERVICE_TYPE as d, servicesService as s };
//# sourceMappingURL=services-BNBgzrtU.js.map
