import { c as create_ssr_component, v as validate_component, h as each, j as add_attribute, e as escape } from './ssr-4574e5c0.js';
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render($$result, { title: "Settings" }, {}, {
    default: () => {
      return `<div class="row overflow-auto"><div class="col"><table class="table table-sm"><colgroup data-svelte-h="svelte-23sk60"><col width="50%"> <col width="50%"> <col> <col> <col></colgroup> <tbody><tr><td><select form="formNew" name="newName" class="form-select form-select-sm">${each(Object.entries(SCREEN_NAMES), ([value, name]) => {
        return `<option${add_attribute("value", value, 0)}>${escape(name)}</option>`;
      })}</select></td> <td>${newName in SCREEN_PARAMS ? `<select form="formNew" name="newParams" class="form-select form-select-sm">${each(Object.entries(SCREEN_PARAMS[newName]), ([value, name]) => {
        return `<option${add_attribute("value", value, 0)}>${escape(name)}</option>`;
      })}</select>` : `<input form="formNew" type="hidden" name="newParams" value="">`}</td> <td></td> <td></td> <td><form id="formNew" method="POST" action="?/add"><button type="submit" class="btn btn-sm btn-success" ${"disabled"}><i class="icofont-ui-add"></i></button></form></td></tr> ${each(screens, (screen, i) => {
...
//# sourceMappingURL=_page.svelte-733132c5.js.map
