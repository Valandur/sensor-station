...
import { format, parseISO, differenceInMinutes, parse, differenceInSeconds } from 'date-fns';
...
const BATTERY_SERVICE_ACTIONS = ["preview", "config", "icon"];
...
const CALENDAR_SERVICE_ACTIONS = ["main", "preview", "config"];
const CAROUSEL_SERVICE_TYPE = "carousel";
const CAROUSEL_SERVICE_ACTIONS = ["main", "preview", "config"];
const EPIC_GAMES_SERVICE_TYPE = "epic-games";
const EPIC_GAMES_SERVICE_ACTIONS = ["main", "preview", "config"];
const GALLERY_SERVICE_TYPE = "gallery";
const GALLERY_SERVICE_ACTIONS = ["main", "preview", "config"];
const HOLIDAY_SERVICE_TYPE = "holiday";
const HOLIDAY_SERVICE_ACTIONS = ["main", "preview", "config"];
const MODEM_SERVICE_TYPE = "modem";
const MODEM_SERVICE_ACTIONS = ["preview", "config", "icon"];
...
const NETWORK_SERVICE_ACTIONS = ["preview", "icon"];
const PRUSA_SERVICE_TYPE = "prusa";
const PRUSA_SERVICE_ACTIONS = ["main", "preview", "config"];
const SBB_ALERTS_SERVICE_TYPE = "sbb-alerts";
const SBB_ALERTS_SERVICE_ACTIONS = ["main", "preview", "config"];
const SBB_DEPARTURES_SERVICE_TYPE = "sbb-departures";
const SBB_DEPARTURES_SERVICE_ACTIONS = ["main", "preview", "config"];
const SRF_SERVICE_TYPE = "srf";
const SRF_SERVICE_ACTIONS = ["main", "details", "preview", "config"];
const SWISS_POST_SERVICE_TYPE = "swiss-post";
const SWISS_POST_SERVICE_ACTIONS = ["main", "preview", "config"];
const TUYA_SERVICE_TYPE = "tuya";
const TUYA_SERVICE_ACTIONS = ["main", "preview", "config"];
...
const DHT_SENSOR_SERVICE_ACTIONS = ["main", "preview", "config"];
class Logger {
...
const ENABLED$f = private_env.BATTERY_ENABLED === "1";
...
const ENABLED$e = private_env.CALENDAR_ENABLED === "1";
...
const ENABLED$d = private_env.CAROUSEL_ENABLED === "1";
class CarouselService extends BaseService {
  ...
  async getData(action, options) {
    ...
    const iconPromises = [];
    for (const screen2 of iconScreens) {
      const service2 = servicesService.getByName(screen2.name);
      iconPromises.push(
        service2.get(screen2.icon, options).then((data) => {
          if (!data) {
            throw new Error();
          }
          return {
...
const ENABLED$c = private_env.DHT_SENSOR_ENABLED === "1";
...
const ENABLED$b = private_env.EPIC_GAMES_ENABLED === "1";
...
const ENABLED$a = private_env.GALLERY_ENABLED === "1";
...
const ENABLED$9 = private_env.HOLIDAY_ENABLED === "1";
class HolidayService extends BaseService {
  static actions = HOLIDAY_SERVICE_ACTIONS;
  type = HOLIDAY_SERVICE_TYPE;
  cache = new Cache(this.logger);
  getDefaultConfig() {
    return {
      country: "",
  ...
  async getData({ url }) {
    ...
    const data = await this.cache.with(
      ...
      async () => {
        ...
        const shipments = rawShipments.filter((s) => s.shipment.globalStatus !== "DELIVERED").map(({ shipment: shipment2 }) => {
          const phys = shipment2.physicalProperties;
          let type = this.getText(shipment2.product) || shipment2.product;
          if (shipment2.internationalProduct) {
            const newType = this.getText(shipment2.internationalProduct);
            if (newType) {
              type = newType;
            }
          }
          return {
            id: shipment2.identity,
            number: shipment2.formattedShipmentNumber ?? "-- unbekannt --",
            type,
            arrival: shipment2.calculatedDeliveryDate ?? null,
            status: null,
            sender: shipment2.debitorDescription ?? null,
            dims: phys.dimension1 ? { x: phys.dimension1, y: phys.dimension2, z: phys.dimension3 } : null,
            weight: phys.weight ?? null
          };
        });
        for (const shipment2 of shipments) {
          ...
          shipment2.status = this.getText(event.eventCode) || event.eventCode;
        }
        return {
          ts: /* @__PURE__ */ new Date(),
          shipments
