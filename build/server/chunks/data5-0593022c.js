...
async function getData(forceUpdate = false) {
  return cache.withDefault(forceUpdate, async () => {
    ...
    const deliveryPayload = resDepartures["trias:Trias"]["trias:ServiceDelivery"][0]["trias:DeliveryPayload"][0];
    const departures = deliveryPayload["trias:StopEventResponse"][0]["trias:StopEventResult"].map((d) => d["trias:StopEvent"][0]).map((d) => ({
...
export { getData as g };
//# sourceMappingURL=data5-0593022c.js.map
