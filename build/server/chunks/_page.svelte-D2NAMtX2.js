...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="h-100 d-flex flex-column justify-content-end">${games.length > 0 ? `<div class="row row-cols-2">${each(games, (game) => {
    return `<div class="col">${validate_component(Card, "Card").$$render($$result, {}, {}, {
      ...
      header: () => {
        return `<div>${escape(format(game.startsAt, "dd MMM", { locale: de }))} -
								${escape(game.endsAt ? format(game.endsAt, "dd MMM", { locale: de }) : "")}</div> <div>${game.pct === 0 ? `Free` : `${escape(game.pct)}%`}</div> `;
      }
    })} </div>`;
...
//# sourceMappingURL=_page.svelte-D2NAMtX2.js.map
