import { c as create_ssr_component, v as validate_component, d as each, k as add_attribute, e as escape } from './index3-53157c6f.js';
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(PageLayout, "PageLayout").$$render($$result, { title: "Settings" }, {}, {
    default: () => {
      return `<div class="row overflow-auto"><div class="col"><table class="table table-sm"><colgroup><col width="50%">
					<col width="50%">
					<col>
					<col>
					<col></colgroup>
        ...
        return `<option${add_attribute("value", value, 0)}>${escape(name)}</option>`;
						...
						<td></td>
						<td></td>
						<td><form id="formNew" method="POST" action="?/add"><button type="submit" class="btn btn-sm btn-success" ${"disabled"}><i class="icofont-ui-add"></i></button></form></td></tr>
        ...
        return `<tr><td>${escape(SCREEN_NAMES[screen.name] || screen.name)}</td>
          ...
          (i < screens.length - 1 ? "btn-outline-theme" : "") + " " + (i === screens.length - 1 ? "btn-outline-secondary" : "")
							...
							<td><form method="POST" action="?/delete"><input type="hidden" name="index"${add_attribute("value", i, 0)}>
									<button class="btn btn-sm btn-danger"><i class="icofont-ui-delete"></i></button>
								</form></td>
						</tr>`;
      })}</tbody></table></div></div>`;
    }
...
//# sourceMappingURL=_page.svelte-45f58ea5.js.map
