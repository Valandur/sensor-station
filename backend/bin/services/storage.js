...
class Storage extends service_1.Service {
    ...
    async runPrepared(query, values) {
        const stmt = await new Promise((res, rej) => this.db.prepare(query, function (err) {
    ...
    async run(query, params) {
        return new Promise((res, rej) => this.db.run(query, params, function (err) {
            err ? rej(err) : res(this.lastID);
        }));
    }
    async all(query, params) {
        return new Promise((res, rej) => this.db.all(query, params, (err, rows) => (err ? rej(err) : res(rows))));
    }
}
